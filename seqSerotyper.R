...
runMview <- function(mviewIn, mviewOut) {
  system2("mview",args=c("-in","blast",mviewIn,"-out","pearson",">",mviewOut))
}
...
###############################
  ...
  if (nrow(assignOtherSero6) > 0) {
    ...
    file6AFlist <- idSero6AFB[,c("checknotSero6CDid", "checknotSero6CD", "V1")]
...
##############
    if (nrow(file6AFlist) > 0) {
      ...
      listSero6AFfasta <- dir(pattern=".6AF.tab.fasta")
      system2("mkdir","6AF")
    
      for (i in 1:length(listSero6AFfasta)) {
        runSplitMultifasta(multifasta = listSero6AFfasta[[i]], outDir = "6AF") 
      }
      ...
      addIDassignSero6Fin <- cbind(checknotSero6CDid, checknotSero6CD, assignSero6F)
      ...
      assignSero6Ffinal <- assignSero6Fout[,c("checknotSero6CDid","V3")]
...
#####################
    ...
    if (exists("assignSero6ABout") == TRUE) {
      ...
      checkSero6AB <- paste(assignSero6ABout$checknotSero6CD)
      checkSero6ABid <- assignSero6ABout$checknotSero6CDid
...
if (nrow(sero18Results)>0) {
  ...
  tab18 <- paste(fasta18ID,".18.tab",sep="")
  n18 <- length(fasta18)
  ...
  tabinSero18 <- dir(pattern=".18.tab")
  sero18out<-NULL
...
if (nrow(sero20Results)>0) {
  ...
  tab20 <- paste(fasta20ID,".20.tab",sep="")
  n20 <- length(fasta20)
  ...
  tabinSero20 <- dir(pattern=".20.tab")
...
if (nrow(sero22Results)>0) {
  ...
  tab22 <- paste(fasta22ID,".22.tab",sep="")
  n22 <- length(fasta22)
...
#######################
  ...
  out37 <- paste(fasta33ID,".37.out",sep="")
  n33 <- length(fasta33)
  ...
  sero37outIn <- dir(pattern=".37.out")
...
#################################
  ...
  tab33 <- paste(fasta33AFid, ".33.tab", sep="")
  ...
  tabinSero33 <- dir(pattern=".33.tab")
...
if (nrow(sero25Results)>0) {
  ...
  tab25 <- paste(fasta25ID, ".25.tab", sep="")
  n25 <- length(fasta25)
  ...
  tabinSero25 <- dir(pattern=".25.tab")
